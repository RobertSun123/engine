...
TEST(AndroidExternalViewEmbedder, SubmitFlutterView) {
  ...
  // ------------------ Third frame ------------------ //
  {
    ...
    // Set a valid vsync_id so the embedder will use AndroidSurfaceTransaction
    SurfaceFrame::SubmitInfo submit_info;
    auto vsync_id = 124;
    submit_info.vsync_id = vsync_id;
    surface_frame->set_submit_info(submit_info);
    EXPECT_CALL(surface_transaction_mock, Begin());
    EXPECT_CALL(surface_transaction_mock, SetVsyncId(vsync_id));
    EXPECT_CALL(surface_transaction_mock, End());

...
void VsyncWaiterAndroid::Loop() {
  switch (NDKHelpers::ChoreographerSupported()) {
    case ChoreographerSupportStatus::kSupportedVsync: {
      auto* weak_this = new std::weak_ptr<VsyncWaiter>(shared_from_this());
      fml::TaskRunner::RunNowOrPostTask(
          task_runners_.GetUITaskRunner(), [weak_this]() {
            NDKHelpers::AChoreographer_postVsyncCallback(
                NDKHelpers::AChoreographer_getInstance(), &OnVsyncFromNDK33,
                weak_this);
          });
    } break;
...
void VsyncWaiterAndroid::OnVsyncFromNDK33(
  ...
  auto frame_time = NormalizeFrameTime(
      NDKHelpers::AChoreographerFrameCallbackData_getFrameTime(callback_data));
  auto* weak_this = reinterpret_cast<std::weak_ptr<VsyncWaiterAndroid>*>(data);
  ...
  // uneven frame pacing.
  auto target_time =
      NDKHelpers::AChoreographerFrameCallbackData_getFrameDeadline(
          callback_data, 1);
  auto next_frame_time =
      frame_time + fml::TimeDelta::FromNanoseconds(g_refresh_period_);
  auto vsync_id = NDKHelpers::AChoreographerFrameCallbackData_getFrameVsyncId(
      callback_data, 1);
